  ...
  var parse_default = (expression) => {
    ...
    const gobbleParameters = (termination) => {
      ...
      while (index2 < expression.length) {
        gobbleSpaces();
        const characterIndex = expression.charCodeAt(index2);
    ...
    const gobbleBinaryOperation = () => {
      gobbleSpaces();
      let toCheck = expression.substring(index2, index2 + 4);
      let toCheckLength = toCheck.length;
      while (toCheckLength > 0) {
        if (Object.hasOwn(BINARY_OPERATORS, toCheck) && (!isIdentifierStart(expression.charCodeAt(index2)) || index2 + toCheck.length < expression.length && !isIdentifierPart(expression.charCodeAt(index2 + toCheck.length))) && !(toCheck === "=" && expression.charCodeAt(index2 + 1) === 62)) {
          index2 += toCheckLength;
          return toCheck;
        }
    ...
    const gobbleArrowFunction = (node) => {
      ...
      if (node && (node.type === IDENTIFIER || node.type === ARRAY || node.type === OBJECT || node.type === ARROW)) {
        if (expression.charCodeAt(index2) === EQUAL_CODE && expression.charCodeAt(index2 + 1) === ANGLE_RIGHT_CODE) {
          index2 += 2;
          gobbleSpaces();
          let body;
          if (expression.charCodeAt(index2) === OPENING_BRACES_CODE) {
        ...
        } else if (expression.charCodeAt(index2) === OPENING_BRACES_CODE) {
    ...
    const gobbleBlockBody = () => {
      ...
      gobbleSpaces();
      if (expression.charCodeAt(index2) === CLOSING_BRACES_CODE) {
        index2++;
      }
      if (nodes2.length === 0) {
        return;
      ...
      while (index2 < expression.length) {
        ...
        if (characterIndex === SEMICOLON_CODE || characterIndex === COMMA_CODE) {
          ...
          if (node) {
            nodes2.push(node);
            if (node.type === RETURN) {
              if (expression.charCodeAt(index2) === SEMICOLON_CODE) {
                index2++;
              }
              break;
            }
          } else if (index2 < expression.length) {
            throw new Error(`Unexpected "${expression.charAt(index2)}"`);
          }
        }
      }
      return nodes2;
    ...
    const gobbleIdentifier = () => {
      let character = expression.charCodeAt(index2);
      const start = index2;
      ...
      while (index2 < expression.length) {
        character = expression.charCodeAt(index2);
      ...
      return {
        type: IDENTIFIER,
        name: expression.slice(start, index2)
      };
    };
    const gobbleNumericLiteral = () => {
      let number = "";
      while (isDecimalDigit(expression.charCodeAt(index2))) {
        number += expression.charAt(index2++);
      }
      if (expression.charCodeAt(index2) === PERIOD_CODE) {
        number += expression.charAt(index2++);
        while (isDecimalDigit(expression.charCodeAt(index2))) {
          number += expression.charAt(index2++);
        }
      }
      let character = expression.charAt(index2);
      if (character === "e" || character === "E") {
        number += expression.charAt(index2++);
        character = expression.charAt(index2);
        ...
        if (!isDecimalDigit(expression.charCodeAt(index2 - 1))) {
          throw new Error(`Expected exponent (${number}${expression.charAt(index2)})`);
        }
      }
      const characterCode = expression.charCodeAt(index2);
      if (isIdentifierStart(characterCode)) {
        throw new Error("Variable names cannot start with a number (" + number + expression.charAt(index2) + ")");
    ...
    const gobbleObjectExpression = () => {
      if (expression.charCodeAt(index2) === OPENING_BRACES_CODE) {
        index2++;
        const properties = [];
        while (!Number.isNaN(expression.charCodeAt(index2))) {
          gobbleSpaces();
          if (expression.charCodeAt(index2) === CLOSING_BRACES_CODE) {
            index2++;
          ...
          gobbleSpaces();
          if (key.type === IDENTIFIER && (expression.charCodeAt(index2) === COMMA_CODE || expression.charCodeAt(index2) === CLOSING_BRACES_CODE)) {
            properties.push({
              type: PROPERTY,
              computed: false,
              key,
          ...
          } else if (expression.charCodeAt(index2) === COLON_CODE) {
            index2++;
            gobbleSpaces();
          ...
          if (expression.charCodeAt(index2) === COMMA_CODE) {
            index2++;
          }
        }
        throw new Error("Missing }");
    ...
    const gobbleRegularExpression = () => {
      if (expression.charCodeAt(index2) === FORWARD_SLASH_CODE) {
        const startIndex = ++index2;
        let inCharSet = false;
        while (index2 < expression.length) {
          if (expression.charCodeAt(index2) === FORWARD_SLASH_CODE && !inCharSet) {
            const pattern = expression.slice(startIndex, index2);
            let flags = "";
            while (++index2 < expression.length) {
              const code = expression.charCodeAt(index2);
              if (code >= LOWER_A_CODE && code <= LOWER_Z_CODE || code >= UPPER_A_CODE && code <= UPPER_Z_CODE || code >= ZERO_CODE && code <= NINE_CODE) {
                flags += expression.charAt(index2);
              } else {
                break;
              }
            }
          ...
          if (expression.charCodeAt(index2) === OPENING_BRACKET_CODE) {
            inCharSet = true;
          } else if (inCharSet && expression.charCodeAt(index2) === CLOSING_BRACKET_CODE) {
            inCharSet = false;
          }
          index2 += expression.charCodeAt(index2) === BACK_SLASH_CODE ? 2 : 1;
        }
    ...
    const gobbleSequence = () => {
      ...
      if (expression.charCodeAt(index2) === CLOSING_PARENTHESIS_CODE) {
        index2++;
        gobbleSpaces();
        if (expression.charCodeAt(index2) === EQUAL_CODE && expression.charCodeAt(index2 + 1) === ANGLE_RIGHT_CODE) {
          index2 += 2;
    ...
    const gobbleSpaces = () => {
      while (SPACE_CODES.includes(expression.charCodeAt(index2))) {
        index2++;
      }
    };
    const gobbleStringLiteral = () => {
      let value = "";
      const quote = expression.charCodeAt(index2++);
      while (index2 < expression.length) {
        const characterCode = expression.charCodeAt(index2++);
        ...
        if (characterCode === BACK_SLASH_CODE) {
          const character = expression.charAt(index2++);
          switch (character) {
            case "n":
              value += `
`;
          ...
          value += expression.charAt(index2 - 1);
        }
      }
    ...
    const gobbleTemplateLiteral = () => {
      ...
      while (index2 < expression.length) {
        ...
        if (characterCode === DOLLAR_CODE && expression.charCodeAt(index2) === OPENING_BRACES_CODE) {
          index2++;
          elements.push(value);
          value = "";
...
`;
    ...
    const gobbleTernary = (node) => {
      if (!node || expression.charCodeAt(index2) !== QUESTION_MARK_CODE) {
        return node;
      }
      index2++;
      ...
      gobbleSpaces();
      if (!expression.charCodeAt(index2) === COLON_CODE) {
    ...
    const gobbleToken = () => {
      ...
      gobbleSpaces();
      const character = expression.charCodeAt(index2);
      if (character === PERIOD_CODE && expression.charCodeAt(index2 + 1) === PERIOD_CODE && expression.charCodeAt(index2 + 2) === PERIOD_CODE) {
        index2 += 3;
        node = {
          type: SPREAD,
        ...
        let toCheck = expression.substring(index2, index2 + 1);
        let toCheckLength = toCheck.length;
        while (toCheckLength > 0) {
          if (UNARY_OPERATORS.includes(toCheck) && (!isIdentifierStart(expression.charCodeAt(index2)) || index2 + toCheck.length < expression.length && !isIdentifierPart(expression.charCodeAt(index2 + toCheck.length)))) {
            index2 += toCheckLength;
    ...
    const gobbleTokenProperty = (node) => {
      ...
      while (character === PERIOD_CODE || character === OPENING_BRACKET_CODE || character === OPENING_PARENTHESIS_CODE || character === QUESTION_MARK_CODE) {
        ...
        if (character === QUESTION_MARK_CODE) {
          if (expression.charCodeAt(index2 + 1) !== PERIOD_CODE) {
            break;
          }
          optional = true;
          index2 += 2;
    ...
    const gobbleUpdatePrefixExpression = () => {
      if (index2 + 1 < expression.length) {
        const characters = expression.substring(index2, index2 + 2);
    ...
    const gobbleUpdateSuffixExpression = (node) => {
      if (!node || index2 + 1 >= expression.length) {
        return node;
      }
      const characters = expression.substring(index2, index2 + 2);
      let operator = null;
  ...
  var run = (node, context = {}) => {
    ...
    switch (node.type) {
      ...
      case TEMPLATE:
        return node.elements.map((element, index2) => element + (index2 < node.expressions.length ? run(node.expressions[index2], context).toString() : "")).join("");
      case UNARY: {
  ...
  var interpret = (component, attribute, expression, extra = null, options = null) => {
    ...
    try {
      ...
      if ((!options || options?.return) && expressionParsed && expressionParsed.length > 1) {
