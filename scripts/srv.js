...
const getMimeType = (filePath) => {
	...
	return mimeTypes[extension] || MIME_TYPE_DEFAULT;
};
 ...
 */
const generateDirectoryListingHTML = (currentLogicalPath, entries) => {
	// Ensure currentLogicalPath starts with a slash for display and consistency.
	const displayPath = currentLogicalPath.startsWith("/")
		? currentLogicalPath
		: "/" + currentLogicalPath;
 ...
 */
const startServer = async (rootDirectory) => {
	const server = http.createServer(async (request, response) => {
		...
		try {
			...
			const requestedLogicalPath = decodeURIComponent(url.split("?")[0]);
			...
			if (
				...
				logWarn(
					`Forbidden: Attempt to access restricted path segment in "${requestedLogicalPath}"`,
				);
				...
				response.end(
					"Forbidden: Access to dotfiles, dot-directories, or node_modules is not allowed.",
				);
				return;
			}
			...
			const effectiveRelativePath = requestedLogicalPath.startsWith("/")
			...
			if (
				...
				logWarn(
					`Forbidden: Path traversal attempt for "${fileSystemPath}" (from "${requestedLogicalPath}")`,
				);
				...
				response.end(
					"Forbidden: Access outside of the served directory is not allowed.",
				);
				return;
			}
			...
			if (stats.isDirectory()) {
				...
				try {
					...
					const htmlListing = generateDirectoryListingHTML(
						requestedLogicalPath,
						entries,
					);
					response.writeHead(200, {
			...
			logError(
				`Unhandled error processing request ${url}: ${error.stack || error.message}`,
			);
...
startServer(targetDirectoryResolved);
