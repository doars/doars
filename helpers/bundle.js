...
const performBuilds = async (builds) => {
	return Promise.all(
		builds.map(async (options) => {
			try {
				...
				if (isProduction && options.minify) {
...
const bundle = async (...builds) => {
	...
	for (let i = builds.length - 1; i >= 0; i--) {
		...
		if (!options.entrypoints || !options.outfile) {
			console.warn(
				"Bundle options are missing entrypoints or outfile properties.",
			);
			// Remove from list.
			builds.splice(i, 1);
			continue;
		...
		if (!isProduction) {
			watchedDirectories.add(path.dirname(options.entrypoints[0]));
		}
	}
